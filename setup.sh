...
persist_devbase_repos() {
  ...
  if [[ "$_DEVBASE_FROM_GIT" == "true" ]]; then
    ...
    if [[ ! -d "$core_dest/.git" ]]; then
      ...
      if git clone --depth 1 --branch "$current_tag" "$current_remote" "$core_dest" 2>/dev/null; then
        ...
        git -C "$core_dest" checkout "$current_tag" --quiet 2>/dev/null || true
        show_progress success "Core repo cloned to $core_dest"
      fi
    else
      # Already exists - update to current tag
      show_progress step "Updating persistent core repo..."
      git -C "$core_dest" fetch --depth 1 origin tag "$current_tag" --quiet 2>/dev/null ||
        git -C "$core_dest" fetch --depth 1 --tags --quiet
      git -C "$core_dest" checkout "$current_tag" --quiet 2>/dev/null || true
