...
check_permissions() {
  ...
  if [[ "$actual" == "$expected" ]]; then
    ...
    elif [[ "$type" == "file" && "$path" =~ id_ed25519 && "$actual" != "600" ]]; then
...
# Add SSH key permissions dynamically based on DEVBASE_SSH_KEY_NAME
if [[ -n "${DEVBASE_SSH_KEY_NAME:-}" ]]; then
  FILE_PERMISSIONS["$HOME/.ssh/${DEVBASE_SSH_KEY_NAME}"]="600"
  FILE_PERMISSIONS["$HOME/.ssh/${DEVBASE_SSH_KEY_NAME}.pub"]="644"
else
  # Fallback to default if variable not set
  FILE_PERMISSIONS["$HOME/.ssh/id_ed25519_devbase"]="600"
  FILE_PERMISSIONS["$HOME/.ssh/id_ed25519_devbase.pub"]="644"
fi
...
check_custom_tools() {
  ...
  # reuse (license compliance tool, installed via pipx)
  custom_total=$((custom_total + 1))
  if has_command "reuse"; then
    print_check "pass" "reuse (License compliance tool)"
    custom_installed=$((custom_installed + 1))
  else
    print_check "info" "reuse (optional, not installed)"
  fi
