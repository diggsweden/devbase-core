...
_install_vscode_ext_install_one() {
  ...
  # Install extension
  local install_cmd="NODE_EXTRA_CA_CERTS=/etc/ssl/certs/ca-certificates.crt $code_cmd"
  [[ -n "$remote_flag" ]] && install_cmd="$install_cmd $remote_flag"
  install_cmd="$install_cmd --install-extension $ext_id --force"
