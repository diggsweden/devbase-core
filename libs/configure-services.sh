...
_is_systemctl_user_bus_missing() {
  local output="$1"

  [[ "$output" == *"No medium found"* || "$output" == *"Failed to connect to bus"* || "$output" == *"No such file or directory"* ]]
}
...
enable_user_service() {
  ...
  if ! daemon_output=$(systemctl --user daemon-reload 2>&1); then
    if _is_systemctl_user_bus_missing "$daemon_output"; then
      show_progress info "${service_desc}: no user session bus — run 'systemctl --user enable ${service_name}' after next login"
      return 0
    fi
    show_progress warning "Failed to reload systemd user daemon"
  fi

  if enable_output=$(systemctl --user enable "$service_name" 2>&1); then
    ...
    if _is_systemctl_user_bus_missing "$enable_output"; then
      show_progress info "${service_desc}: no user session bus — run 'systemctl --user enable ${service_name}' after next login"
      return 0
    fi
