...
├── post-commit.d/
│   └── 01-conventional-commits.sh # Active: Conventional commits via gommitlint defaults
...
* **01-conventional-commits.sh** - Validates commit policy using gommitlint. Uses gommitlint defaults automatically, and applies project-specific rules from `.gommitlint.yaml` when present.
...
6. **post-commit** dispatcher validates commit policy non-blocking, using gommitlint defaults and optional project config
...
**Enforcement:** Use repository policies (for example, gommitlint in CI) to ensure rebased commits still comply.
