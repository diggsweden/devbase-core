...
- `setup.sh` is the entrypoint; call `init_env` before using shared libs, `libs/bootstrap.sh` prepares UI/env, `libs/install.sh` assumes shared libs are sourced there
