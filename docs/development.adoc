...
- `setup.sh` is the entrypoint; call `init_env` before using shared libs, `libs/bootstrap.sh` orchestrates bootstrap while `libs/bootstrap-ui.sh` and `libs/bootstrap-config.sh` provide helpers, `libs/install.sh` assumes shared libs are sourced there
