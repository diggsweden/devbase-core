...
function __update_wt_apply_scheme --description "Apply Windows Terminal color scheme"
    ...
    # First validate that the input settings file is valid JSON
    if not jq empty "$wt_settings" 2>/dev/null
        echo "✗ Windows Terminal: settings.json is not valid JSON" >&2
        echo "  File: $wt_settings" >&2
        rm -f "$temp_file"
        return 1
    end
    ...
    if not mv "$temp_file" "$wt_settings" 2>/dev/null
        echo "✗ Windows Terminal: Failed to write settings file (permission denied?)" >&2
...
function update-windows-terminal-theme --description "Update Windows Terminal color scheme (WSL only)"
    ...
    set -l wt_settings $settings_data[1]
    set -l backup_file $settings_data[2]
